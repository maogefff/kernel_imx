...
static int eeprom_write_mode[MCHIP_MAX_USER_DATA+1]={0};
		       ...
		       const char *buf, size_t count)
{
	...
	{
		if(calibrationSetting<10000 && calibrationSetting>=0 && XFactory!=0 && YFactory!=0 && Version!=calibrationSetting)
		{
			unsigned char buff[16]={0}; 	

			Version=calibrationSetting;
			eeprom_write_mode[6]=1;
			buff[0]=0x55;
			buff[1]=0x06;
			buff[2]=0x29;
			buff[3]=0x00;
			...
			buff[5]=0x02;
			buff[6]=0xff&Version;
			buff[7]=0xff&(Version>>8);
			for (i=0;i<8;i++)
			{
				spi_write(privRef->client,(unsigned char *)&buff[i],1);
			}
			msleep(5);
			eeprom_write_mode[6]=0;
		}
	}
#else
	else if (strcmp(attr->attr.name, "minX") == 0)
...
static struct kobj_attribute Version_attribute =
	__ATTR(Version, 0660, calibrationSettings_show, calibrationSettings_store);
#else
...
static void ar1021_spi_readdata(struct work_struct *work)
{
	...
	for(i=0;i<MCHIP_MAX_USER_DATA;i++)
	...
	if(eeprom_read_mode_flag)
	{
		...
		switch(eeprom_read_mode_flag)
		{
			case 1:
				...
				printk("ar1021 SPI(%d): XFactory: %d",eeprom_read_mode_flag,XFactory);
				break;
			case 2:
				if((buff[5]&0x80)==0x80)
				{
				...
				printk("ar1021 SPI(%d): YFactory: %d",eeprom_read_mode_flag,YFactory);
				break;
			case 3:
				if((buff[5]&0x10)==0x10)
				{
				...
				printk("ar1021 SPI(%d): XLorg: %d",eeprom_read_mode_flag,XLorg);
				break;
			case 4:
				if((buff[5]&0x10)==0x10)
				{
				...
				printk("ar1021 SPI(%d): YLorg: %d",eeprom_read_mode_flag,YLorg);
				break;
			case 5:
				if((buff[5]&0x10)==0x10)
				{
				...
				printk("ar1021 SPI(%d): Xorg: %d",eeprom_read_mode_flag,Xorg);
				break;
			case 6:
				if((buff[5]&0x10)==0x10)
				{
				...
				printk("ar1021 SPI(%d): Yorg: %d",eeprom_read_mode_flag,Yorg);
				break;
			case 7:
				if((buff[5]&0x80)==0x80)
				{
				...
				if(ret<0 || ret >9999)
					goto five_point_error_return;
				Version=ret;
				printk("ar1021 SPI(%d): Version: %d",eeprom_read_mode_flag,Version);
				break;
		}
		if(eeprom_read_mode_flag<MCHIP_MAX_USER_DATA)
		{
		...
		{
			printk("ar1021 SPI(%d): read user date completed.",eeprom_read_mode_flag);
...
//				test_irq_handler_func,IRQF_TRIGGER_RISING, "ar1021 IRQ", priv)))
	...
	echo "0x55 0x04 0x28 0x00 0x90 0x04">/sys/kernel/ar1021/sendBuffer
	echo "0x55 0x04 0x28 0x00 0x94 0x04">/sys/kernel/ar1021/sendBuffer
	*/
